...
export default function NewLayout({ 
  ...
  useEffect(() => {
    if (editReviewId && placeContext.cardPlace && user?.id) {
      console.log('🔧 Edit review requested from URL:', editReviewId);
      // Wait a bit for the place card to fully load, then open the review form
      const timer = setTimeout(() => {
        // The place card should have loaded the user's existing review by now
        // We'll let the place card handle opening the form with the existing review data
        console.log('🔧 Triggering review form open for edit review:', editReviewId);
        // The form will be opened by the place card's existing logic
      }, 1000);
      
      return () => clearTimeout(timer);
    }
  }, [editReviewId, placeContext.cardPlace, user?.id]);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      />
       ...
       <FilterBar
         selectedAllergens={filterStore.selectedAllergens}
         selectedRating={filterStore.selectedMinimumRating}
      ...
      <div className="flex h-[calc(100vh-140px)]">
        ...
        {isDesktop && (
          <>
            ...
            {/* Right Panel - Map (75% when no place card, 50% when place card open) */}
            <div className={`relative ${cardPlace ? 'w-1/2' : 'w-3/4'}`}>
              <LazyMapContainer
                places={mapPlaces}
                center={mapCenter}
                zoom={mapZoom}
                ...
                hoveredMarker={highlightedPlace}
                setHoveredMarker={placeContext.setHighlightedPlace}
                onMapLoad={undefined}
        ...
        {isTablet && (
          <div className="w-full relative">
            <LazyMapContainer
              ...
              setHoveredMarker={placeContext.setHighlightedPlace}
              onMapLoad={undefined}
              onPlaceCardOpen={handlePlaceSelect}
              activeLocation={activeLocation}
            />
            ...
            <MobileFloatingButton
              ...
              windowWidth={windowWidth}
              cardPlace={cardPlace}
              showMobileListView={showMobileList}
              setShowMobileListView={setShowMobileList}
            />
